...
- **Complete vs incomplete status**: derived purely from `moves.count == 64` — no reintroduced move-legality logic. Step 3's live visual pass briefly relabeled the shorter-than-64 status "Stuck" (since we can't cheaply verify a genuine dead-end without legality logic) but that was reverted right after: "Incomplete" is the accurate label regardless of *why* a tour is short — abandoned or genuinely stuck both just mean "not done" — so it's back to "Incomplete", now with its own yellow `--color-status-incomplete` token instead of reusing the red `--color-board-visited` (which stays scoped to its original in-play dead-end/trail meaning).
...
- **Status pill**: label is "Incomplete" (briefly "Stuck", reverted — see the decision above). Colors: `--color-board-legal` (Complete, reused) / new `--color-status-incomplete` yellow (Incomplete) — deliberately *not* `--color-board-visited`, since that red is the in-play dead-end/trail signal and conflating it with "not yet 64 moves" was the original mistake. Both `text-zinc-950` for contrast. The path's start/end dots keep `--color-board-legal`/`--color-board-visited` unconditionally (start/finish markers, not a status indicator) so they no longer visually "match" the pill — that coupling wasn't actually meaningful.
