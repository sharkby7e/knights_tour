...
**Followup**: path line now hides on a stuck/failed board (`renderPath` gets `[]` when `game.stuck`), same as the toggle-off case — since `stuck` is computed live off the cursor position, scrubbing back off the dead end restores it. `bundle exec rspec` 51/51, `node --test` 45/45, `bin/rubocop` clean.

**Followup, consistency audit**: diffed both pages block-by-block. Fixed four structural mismatches: live's column wrapper had an `items-center` playback's didn't (moved that centering onto the Save button via `self-center` instead, so both wrappers are now byte-identical); a now-redundant `w-full` on the path-toggle row; an inert `space-y-6` on the outer wrapper; an unnecessary extra `<div>` wrapping live's board that playback's board partial doesn't have. Also added one functional gap: live play had no keyboard scrubbing while playback did — added the same `keydown@window` action (ArrowLeft/Right → prev/next). `bundle exec rspec` 51/51, `node --test` 45/45, `bin/rubocop` clean.

**Followup**: sized playback's `<h1>Tour #id</h1>` to match live play's status text (`h-10 lg:h-24`, `text-lg lg:text-3xl`), dropping the old `scale-90 lg:scale-100` trick, per the user wanting the two pages to feel like "basically the same page." Also removed the status text's `animate-count-roll` roll-in effect on request (JS trigger + now-dead CSS keyframe/token, both fully removed — nothing else referenced them). Caught and fixed a live bug while in there: `tour_controller.js`'s `render()` still hardcoded `lg:w-64` on every render, silently undoing the `lg:w-full` fix from the layout-consolidation step above. `bundle exec rspec` 51/51, `node --test` 45/45, `bin/rubocop` clean.
